...
def replace_notion_x_page_body(page_id: str, post_data: dict, extra_payload: dict):
    ...
    for image_url in post_data.get("image_urls") or []:
        absolute_url = image_url if str(image_url).startswith("http") else build_public_url(image_url)
        blocks.append(
            {
                "object": "block",
                "type": "image",
...
def create_app():
    ...
    allowed_origins = [r"http://localhost:\d+", r"http://127\.0\.0\.1:\d+"]
    allowed_origins.extend(CORS_ALLOWED_ORIGINS)
    CORS(
        app,
